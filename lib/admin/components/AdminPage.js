...
class AdminPage extends React.Component<Props> {
  ...
  _getMainContent = (isApplicationAdmin: boolean): React.Node => {
    ...
    const restricted = <p className='text-center lead'>Ograniczony dostęp</p>
  ...
  _getLinks = (isApplicationAdmin: boolean): React.Node[] => {
    const links = [this._createLink('/admin/users', 'Zarządzanie użytkownikami')]
    // Do not show non-appAdmin users these application-level settings
    if (isApplicationAdmin) {
      if (!isModuleEnabled('enterprise')) {
        links.push(this._createLink('/admin/organizations', 'Organizacje'))
      }
      links.push(this._createLink('/admin/logs', 'Logi aplikacji'))
      if (isModuleEnabled('deployment')) {
        links.push(this._createLink('/admin/servers', 'Serwery wdrożenia'))
      }
    }
