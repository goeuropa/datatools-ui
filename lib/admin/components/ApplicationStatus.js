...
class ApplicationStatusView extends Component<Props, {refreshTime: ?Date}> {
  ...
  render () {
    ...
    return (
      <div>
        <h3>
          Aktywne prace serwera
          <Button
            className='pull-right'
            onClick={this._refreshJobs}>
            <Icon type='refresh' /> Odśwież
          </Button>
          <br />
          <small>ostatnio zaktualizowane {moment(this.state.refreshTime).format(TIME_FORMAT)}</small>
        </h3>
        <BootstrapTable
          {...jobTableOptions}
          headerStyle={{
        ...
        <h3>
          Ostatnie zapytania do API według użytkownika
          <br />
          <small>ostatnio zaktualizowano{moment(this.state.refreshTime).format(TIME_FORMAT)}</small>
        </h3>
        <BootstrapTable
          {...defaultTableOptions}
        ...
        <h3>Logi aplikacji użytkowników</h3>
        <Button
          bsStyle='danger'
          bsSize='large'
          block
          href='https://manage.auth0.com/#/logs'>
          <Icon type='star' /> Zobacz logi użytkowników na Auth0.com
        </Button>
      </div>
    )
  }
