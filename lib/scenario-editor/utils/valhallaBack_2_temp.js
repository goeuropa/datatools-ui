...
function handleGraphHopperRouting (path, individualLegs = false) {
  // console.info("path, individualLegs:", path, individualLegs);
...
export async function polyline (points, individualLegs = false, avoidMotorways = false) {
  ...
  const geometry = []
  try {
    // Chunk points into sets no larger than the max # of points allowed by GraphHopper plan.
    ...
    for (let i = 0; i < j; i += chunk) {
      ...
      const chunkedPoints = points.slice(beginIndex, endIndex) //* Divided by max pointLimit sizes
      // console.info("chunkedPoints:", chunkedPoints);
      //* Fetching data, params: chunkedPoints, avoidMotorways
    ...
    if (points) {
      ...
      console.info('valhallaData:', valhallaData) //* -> [ 17.01566, 52.08935 ]["segment"][]
    }

    // console.info("geometry:", geometry); //* [ 17.01566, 52.08935 ]["segment"][]
    return geometry
  } catch (e) {
    console.error(e)
    return null
...
export async function getSegment (points, followRoad, defaultToStraightLine = true, avoidMotorways = false) {
  ...
  if (followRoad) {
    ...
    // console.info("coordinates:", coordinates); //* [ 17.25307, 52.23207 ][]
    ...
    // If not snapping to streets, simply generate a line string from input coordinates.
    geometry = lineString(points).geometry
  }
  console.info('geometry:', geometry) //* { type: "LineString", coordinates:[ 17.25307, 52.23207 ][]}
  //  return geometry;
  const valhallaGeometry = await getSegment2(points, followRoad, defaultToStraightLine, avoidMotorways)
...
// - Fetch Data - not used outside this file
...
// + Used in: stropStrategies.js and map.js
export async function getSegment2 (points, followRoad, defaultToStraightLine = true, avoidMotorways = false) {
  ...
  if (followRoad && objectPoints && objectPoints.length) {
    ...
    const decodedChunkedData = encodedResults.shapes.map((shape) => decodeValhallaPolyline(shape, 6))
    ...
    const coordinates = decodedDataConverted.flat(1)
